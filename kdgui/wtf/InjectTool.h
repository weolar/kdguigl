...
extern "C" LPVOID __cdecl ScanCode(LPVOID pStart, DWORD dwLen, const BYTE pChar[], DWORD dwCharLen, FUNC_EXT pFunc = NULL);

// 根据指定地址，倒着查找以返回函数头，找不到返回NULL
extern "C" LPVOID __cdecl ReverseScanFuncHead(LPVOID p, DWORD dwFindLen);

// 替换一个函数，并分配一块内存，把原函数保存下来
extern "C" LPVOID __cdecl ReplaceFuncAndCopy(LPVOID lpOldFunc, LPVOID lpNewFunc);
#endif
