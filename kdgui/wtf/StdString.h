...
public:
	...
	// They also are compiled if you have explicitly turned off refcounting
		...
		MYTYPE& assign(MYCITER iterFirst, MYCITER iterLast)
		{
			...
			if ( iterFirst < begin() || iterFirst > begin() + size() )
				erase();
	#endif
			replace(std::basic_string<CT>::begin(), std::basic_string<CT>::end(), iterFirst, iterLast);
			return *this;
		}
	#endif
	...
	MYTYPE& ToUpper()
	{
		...
		if ( !std::basic_string<CT>::empty() )
			ssupr(GetBuf(), std::basic_string<CT>::size());
	...
	MYTYPE& ToLower()
	{
		...
		if ( !std::basic_string<CT>::empty() )
			sslwr(GetBuf(), std::basic_string<CT>::size());
	...
	CT* GetBuf(int nMinLen=-1)
	{
		...
		return std::basic_string<CT>::empty() ? const_cast<CT*>(std::basic_string<CT>::data()) : &(std::basic_string<CT>::at(0));
	}

	CT* SetBuf(int nLen)
	{
		nLen = ( nLen > 0 ? nLen : 0 );
		if ( std::basic_string<CT>::capacity() < 1 && nLen == 0 )
			std::basic_string<CT>::resize(1);

		resize(static_cast<MYSIZE>(nLen));
		return const_cast<CT*>(std::basic_string<CT>::data());
	}
	void RelBuf(int nNewLen=-1)
	{
		resize(static_cast<MYSIZE>(nNewLen > -1 ? nNewLen : sslen(std::basic_string<CT>::c_str())));
	}
	...
	{
		return sscoll(std::basic_string<CT>::c_str(), std::basic_string<CT>::length(), szThat, sslen(szThat));
	}

	int CollateNoCase(PCMYSTR szThat) const
	{
		return ssicoll(std::basic_string<CT>::c_str(), std::basic_string<CT>::length(), szThat, sslen(szThat));
	}
	...
	{
		return ssicmp(std::basic_string<CT>::c_str(), szThat);
	}
	...
	const CT* GetString() const
	{
		return std::basic_string<CT>::c_str();
	}
	...
	int GetLength() const
	{
		return static_cast<int>(std::basic_string<CT>::length());
	}

	
	int Insert(int nIdx, CT ch)
	{
		if ( static_cast<MYSIZE>(nIdx) > std::basic_string<CT>::size() -1 )
			append(1, ch);
		else
	...
	{
		if ( nIdx >= std::basic_string<CT>::size() )
			append(sz, sslen(sz));
		else
	...
	bool IsEmpty() const
	{
		return std::basic_string<CT>::empty();
	}
	...
	void MakeReverse()
	{
		std::reverse(std::basic_string<CT>::begin(), std::basic_string<CT>::end());
	}
	...
	{
		int nReplaced	= 0;
		for ( MYITER iter=std::basic_string<CT>::begin(); iter != std::basic_string<CT>::end(); iter++ )
		{
			if ( *iter == chOld )
			{
				*iter = chNew;
	...
	int Replace(PCMYSTR szOld, PCMYSTR szNew)
	{
		...
		{
			replace(std::basic_string<CT>::begin()+nIdx, std::basic_string<CT>::begin()+nIdx+nOldLen, szRealNew);
			nReplaced++;
			nIdx += nNewLen;
		}
		return nReplaced;
	...
	{
		nCount = SSMIN(nCount, static_cast<int>(std::basic_string<CT>::size()));
		return substr(std::basic_string<CT>::size()-static_cast<MYSIZE>(nCount));
	}

	void SetAt(int nIndex, CT ch)
	{
		ASSERT(std::basic_string<CT>::size() > static_cast<MYSIZE>(nIndex));
	...
	MYTYPE& TrimLeft()
	{
		erase(std::basic_string<CT>::begin(), std::find_if(std::basic_string<CT>::begin(),std::basic_string<CT>::end(),NotSpace<CT>(std::locale())));
		return *this;
	}
	...
	{
		std::locale loc;
		MYRITER it = std::find_if(std::basic_string<CT>::rbegin(), std::basic_string<CT>::rend(), NotSpace<CT>(loc));
		if ( std::basic_string<CT>::rend() != it )
			erase(std::basic_string<CT>::rend() - it);

		erase(it != std::basic_string<CT>::rend() ? find_last_of(*it) + 1 : 0);
		return *this;
	}
