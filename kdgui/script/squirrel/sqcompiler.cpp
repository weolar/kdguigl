...
#pragma warning(pop)
	...
	bool Compile(SQObjectPtr &o)
	{
		...
		_fs->AddParameter(_fs->CreateString(_SC("arguments"))); // weolar vargv
		_fs->_varparams = true;
	...
	void ParseTableOrClass(SQInteger separator,SQInteger terminator)
	{
		...
		while(_token != terminator) {
            ...
            case TK_IDENTIFIER:  // JSON weolar
				if(separator == ',') { //only works for tables
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(Expect(_token/*TK_STRING_LITERAL*/)));
                    if (_SC('=') == _token) Expect(_SC('=')); else Expect(_SC(':')); Expression();
					break;
				}
			default :
				_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(Expect(TK_IDENTIFIER)));
...
// 		}
        ...
        // 把默认一个值的变量输出，本来表示值名，现在改成属性名
        Lex(); Expect(_SC('(')); idxname = Expect(TK_IDENTIFIER); // valname = Expect(TK_IDENTIFIER);
        if(_token == _SC(',')) { // 如果有逗号，表示一个值名一个属性
            Lex(); valname = Expect(TK_IDENTIFIER);
        } else {
            valname = _fs->CreateString(_SC("@VALUE@"));
        }
		Expect(TK_IN);
	...
	void FunctionStatement()
	{
		...
		_fs->PushTarget(0);
        //if (_fs->IsConstantExist(id)) Error(_SC("function name (%s) is repetition"), _string(id));
		_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
	...
	void CreateFunction(SQObject &name,bool lambda = false)
	{
		...
		while(_token!=_SC(')')) {
			if(_token == TK_VARPARAMS) {
				...
				funcstate->AddParameter(_fs->CreateString(_SC("arguments"))); // weolar vargv
