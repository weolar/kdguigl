...
static SQInteger array_shift(HSQUIRRELVM v) // weolar 在数组的头部删除一个元素
{
    SQObject &o = stack_get(v, 1);
    SQObjectPtr val;
    if(_array(o)->Get(0, val)) {
        _array(o)->Remove(0);
        v->Push(val);
        return 1;
    }
...
static SQInteger array_unshift(HSQUIRRELVM v) // weolar 在数组的头部添加一个元素
{
    SQObject &o=stack_get(v,1);
    SQObject &val=stack_get(v,2);
    if(!_array(o)->Insert(0,val))
        return sq_throwerror(v,_SC("index out of range"));
    return 0;
}
...
// There isn't any limit on the number of 'or' that can be used. 
// Spaces are ignored so can be inserted between types to increase readbility. 
// For instance to check a function that espect a table as 'this' a string as first 
// parameter and a number or a userpointer as second parameter,
// the string would be "tsn|p" (table,string,number or userpointer). 
// If the parameters mask is contains less parameters than 'nparamscheck' 
// the remaining parameters will not be typechecked. 
...
SQRegFunction SQSharedState::_array_default_delegate_funcz[]={
	...
	{_SC("pop"),array_pop,1, _SC("a")}, // 弹出数组末尾元素,数组长度减一
	{_SC("top"),array_top,1, _SC("a")}, // 弹出数组末尾元素,数组长度不变
	{_SC("insertAt"),array_insert,3, _SC("an.")},
	{_SC("removeAt"),array_remove,2, _SC("an")},
    {_SC("shift"),array_shift,1, _SC("a")}, // 在数组的头部删除一个元素
    {_SC("unshift"),array_unshift,2, _SC("a.")}, // 在数组的头部添加一个元素
