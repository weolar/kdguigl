...
#define PREV_CHAR (_prevdata) // weolar
...
void SQLexer::Init(SQSharedState *ss, SQLEXREADFUNC rg, SQUserPointer up,CompilerErrorFunc efunc,void *ed)
{
    ...
    //ADD_KEYWORD(kdcall, TK_FUNCTION);
...
//             {
//                 LexChar prevdata = PREV_CHAR;
//                 NEXT(); // 把对象的声明，改成":"形式
//                 if (CUR_CHAR != _SC(':')) {
//                     if (_SC('t') == CUR_CHAR)
//                     {__asm int 3;
//                     }
//                     if (_SC(')') == prevdata) // 这是三目表示式
//                     {RETURN_TOKEN(':')}
//                     else if (TK_DEFAULT == _prevtoken || TK_CASE == _prevtoken) // 这是swich case
//                     {RETURN_TOKEN(':')}
//                     else{RETURN_TOKEN('=')} // 这是对象的申明表达式
//                 } else { NEXT(); RETURN_TOKEN(TK_DOUBLE_COLON); }
//             }
		case _SC('*'):
			NEXT();
		...
		default:{
				...
				else if (scisalpha(CUR_CHAR) || CUR_CHAR == _SC('_') || CUR_CHAR == _SC('$')) { // weolar
					SQInteger t = ReadID();
					RETURN_TOKEN(t);
				}
				else {
...
SQInteger SQLexer::ReadID()
{
	...
	do {
		APPEND_CHAR(CUR_CHAR);
		NEXT();
	} while(scisalnum(CUR_CHAR) || CUR_CHAR == _SC('_') || CUR_CHAR == _SC('$')); // weolar
	TERMINATE_BUFFER();
