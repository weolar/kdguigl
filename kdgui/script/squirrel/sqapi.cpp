...
{ \
	if(sq_gettop(v) < count){ g_sq_error_result = 6; v->Raise_Error(_SC("not enough params in the stack")); return SQ_ERROR; }\
}		
...
static SQRESULT _sq_rawget(HSQUIRRELVM v,SQInteger idx,bool needThrowerror)
{
	SQObjectPtr &self=stack_get(v,idx);
	switch(type(self)) {
	case OT_TABLE:
...
SQRESULT sq_rawexists(HSQUIRRELVM v,SQInteger idx)
{
    return _sq_rawget(v,idx,false);
}

SQRESULT sq_rawget(HSQUIRRELVM v,SQInteger idx)
{
    return _sq_rawget(v,idx,true);
}
...
SQRESULT sq_compilebuffer(HSQUIRRELVM v,const SQChar *s,SQInteger size,const SQChar *sourcename,SQBool raiseerror) {
    ...
    UINT byteSize = size * sizeof(SQChar);
    int nPageNum = byteSize/dwPageSize + 1;
    if (nPageNum > 10 || nPageNum * dwPageSize < byteSize)
    	return sq_compile(v, buf_lexfeed, &buf, sourcename, raiseerror);

    DWORD dwAllocSize = nPageNum * dwPageSize;
    void* newAddress = ::VirtualAlloc(NULL, dwAllocSize, MEM_COMMIT|MEM_RESERVE, PAGE_READWRITE);
    if (!newAddress)
        return sq_compile(v, buf_lexfeed, &buf, sourcename, raiseerror);
    memcpy(newAddress, s, byteSize);

    DWORD flOldProtect = 0;
    if (!::VirtualProtect(newAddress, dwAllocSize, PAGE_READONLY, &flOldProtect)) {
        ::VirtualFree(newAddress, dwAllocSize, MEM_RELEASE);
        return sq_compile(v, buf_lexfeed, &buf, sourcename, raiseerror);
    }
    
    buf.buf = (const SQChar *)newAddress;
    SQRESULT ret = sq_compile(v, buf_lexfeed, &buf, sourcename, raiseerror);
    //::VirtualProtect(newAddress, size, flOldProtect, &flOldProtect);
    ::VirtualFree(newAddress, dwAllocSize, MEM_RELEASE);
