...
bool SQVM::StartCall(SQClosure *closure,SQInteger target,SQInteger args,SQInteger stackbase,bool tailcall)
{
	...
	if (closure->_function->_bgenerator) {
		...
		if(!gen->SqYield(this,f->_stacksize))
			return false;
		SQObjectPtr temp;
		...
		for(;;)
		{
			...
			case _OP_YIELD:{
				if(ci->_generator) {
					...
					_GUARD(ci->_generator->SqYield(this,arg2));
					traps -= ci->_etraps;
...
bool SQVM::Set(const SQObjectPtr &self,const SQObjectPtr &key,const SQObjectPtr &val,SQInteger selfidx)
{
	...
	case OT_ARRAY:
		...
		return true;
    case OT_STRING: // weolar
        if(sq_isnumeric(key) && sq_isnumeric(val)) {
            SQInteger n = tointeger(key);
            SQInteger nVal = tointeger(val);
            if(abs((int)n) < _string(self)->_len) {
                if(n < 0) n = _string(self)->_len - n;
                _stringval(self)[n] = nVal;
                return true;
            }
            Raise_IdxError(key);
            return false;
        } else
            Raise_Error(_SC("key or val is not numeric or char"));
        return false;
	default:
