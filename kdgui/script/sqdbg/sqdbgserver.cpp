...
SQDBGString& replace_all(SQDBGString& str, const SQDBGString& old_value, const SQDBGString& new_value)   
{   
    while(true) {     
		if((str.Find(old_value)) != -1)
...
bool SQDbgServer::ParseWatch(const char *msg,Watch &out)
{
	...
	while((*ep)!='\n' && (*ep)!='\0')
	{
		out._exp.AppendChar((wchar_t)*ep);
		*ep++;
	}
	return true;
}
...
void SQDbgServer::AddBreakpoint(BreakPoint &bp)
{
	...
	BeginDocument();
		BeginElement(_SC("addbreakpoint"));
			...
			Attribute(_SC("src"),bp._src.GetString());
...
void SQDbgServer::SerializeState(HSQUIRRELVM v)
{
	...
	for(WatchSetItor i=_watches.begin(); i!=_watches.end(); ++i)
	{
		...
		sq_pushstring(v,i->_exp.GetString(),(SQInteger)i->_exp.GetLength());
		sq_createslot(v,-3);
	}
	sq_rawset(v,-3);
