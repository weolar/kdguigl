...
BOOL sq_rdbg_isrunning(HSQREMOTEDBG rdbg)
{
    return (BOOL)(SQDbgServer::eDBG_Running == rdbg->_state);
}
...
SQRESULT sq_rdbg_update(HSQREMOTEDBG rdbg)
{
	...
	if(FD_ISSET(rdbg->_endpoint,&read_flags)){
		...
		char* temp = new char[tempBufSize];
		int size=0;
		char c,prev=NULL;
		memset(temp,0,sizeof(char)*tempBufSize);
		int res;
		...
		while((res = recv(rdbg->_endpoint,&c,1,0))>0){
			...
			if(size >= (int)tempBufSize-2) break;
		}
		switch(res){
