...
BitmapPlatformDevice::BitmapPlatformDeviceData::~BitmapPlatformDeviceData() {
  ...
  BitmapCache::GetTLS()->Recycle(bitmap_context_);
}
...
HDC BitmapPlatformDevice::BitmapPlatformDeviceData::GetBitmapDC() {
  if (!hdc_) {
	...
	BitmapCache::GetTLS()->Recycle((HBITMAP)old_bitmap);
  }
...
BitmapPlatformDevice* BitmapPlatformDevice::create(int width, int height,
	...
	HBITMAP hbitmap = BitmapCache::GetTLS()->Create(width, height, &data, shared_section);
	if (!hbitmap)
		return NULL;
...
// data.
HBITMAP BitmapPlatformDevice::createBitMap(
    HDC screen_dc,
    int width,
    int height,
